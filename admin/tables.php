...
$query=mysqli_query($con,"delete from client where client_no='$postid'");
if($query)
{
$msg="Post deleted ";
}
    ...
    <div class="relative w-full flex flex-col h-screen overflow-y-hidden">
        ...
        <div class="w-full h-screen overflow-x-hidden border-t flex flex-col">
            <main class="w-full flex-grow p-6">
                <h1 class="text-3xl text-black pb-6">Contact</h1>
                
                <div class="w-full mt-6">
                    <p class="text-xl pb-3 flex items-center">
                        <i class="fas fa-list mr-3"></i> Latest Contact
                    </p>
                        ...
                        <table class="min-w-full bg-white">
                            <thead class="bg-gray-800 text-white">
                                <tr>
                                    ...
                                    <th class="text-left py-3 px-4 uppercase font-semibold text-sm">Message</td>
                                    <th class="text-left py-3 px-4 uppercase font-semibold text-sm">Time</td>
                                    <th class="text-left py-3 px-4 uppercase font-semibold text-sm">Action</td>
                                </tr>
                            </thead>
                            <?php 
                $query=mysqli_query($con,"select client_no,client_name,client_email,client_message,client_time from client");
                while($row=mysqli_fetch_array($query))
                {
                ?>
                            <tbody class="text-gray-700">
                                <tr>
                                    <td class="w-1/3 text-left py-3 px-4"><?php echo htmlentities($row['client_no']);?></td>
                                    <td class="w-1/3 text-left py-3 px-4"><?php echo htmlentities($row['client_name']);?></td>
                                    <td class="w-1/3 text-left py-3 px-4"><?php echo htmlentities($row['client_email']);?></td>
                                    <td class="w-1/3 text-left py-3 px-4"><?php echo htmlentities($row['client_message']);?></td>
                                    <td class="w-1/3 text-left py-3 px-4"><?php echo htmlentities($row['client_time']);?></td>
                                    <td><a href="tables.php?pid=<?php echo htmlentities($row['client_no']);?>&&action=del" onclick="return confirm('Do you realy want to delete ?')"> <svg class="h-6 w-6" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
